...
- **Low-click:** Every staff action is ≤ 5 taps from the POS home screen. Job Card advancement is 1 tap.
...
│   │   └── loyalty.py         ← Points, tiers, streaks
│   ├── /integrations/
...
- `customer_comments`
- `streak_progress_text` (e.g. "3/4 weeks — one more for double points!")
...
- `customer` → Laundry Customer (1-to-1)
...
2. For Silver/Gold tier customers: sort eligible machines by load ASC (priority queue)
3. For each eligible machine m:
     if (m.current_load_kg + order.total_weight_kg) <= m.capacity_kg:
         ...
         assign machine, set order.eta, break
...
earn_points():
    points = max(
        ...
        order.grand_total × settings.points_per_currency_unit
    )
...
check_streak(account):
    if last_order_date was within 7 days of previous order:
        account.current_streak_weeks += 1
    else:
        account.current_streak_weeks = 1
    order.streak_progress_text = f"{streak}/{settings.streak_weeks_required} weeks..."
    if streak >= settings.streak_weeks_required:
        award double points bonus via Loyalty Transaction (Bonus type)
        reset streak to 0
```
...
doc_events = {
    ...
    "Laundry Order": {
        ...
        # fires send_payment_thanks only when payment_status changes to Paid
    },
    "Laundry Job Card": {
        "on_submit": "spinly.logic.inventory.deduct_consumables",
        "on_update": [
            "spinly.logic.machine.update_countdown",       # on Running
            "spinly.integrations.whatsapp_handler.send_pickup_reminder",  # on Ready
            "spinly.logic.loyalty.issue_scratch_card",    # on Ready
            "spinly.logic.loyalty.check_streak_on_deliver" # on Delivered
        ]
    },
...
|---|---|---|
| New order for existing customer | ≤ 5 from POS home | Manual tap count |
| New order for new customer | ≤ 7 from POS home | Manual tap count |
...
| Silver tier customer | ETA bumped up (lower queue priority) |
| Gold tier customer | ETA bumped up (lower queue priority) |
